...
def write_ticket(csv_writer, ticket, candidate_tags):
    """
    ...
    # ticket_text = preprocess_text(ticket_title + " " + ticket_content)
    ...
    if len(ticket_taggers) == 1 and "0" in ticket_taggers:
        return
    ...
    if len(ticket_tags) > 0:
        for candidate_tag in candidate_tags:
            if candidate_tag in ticket_tags:
...
def ticket_to_csv(json_path, csv_path):
    ...
    csv_columns = ["ticket_id", "ticket_title", "ticket_content"] + candidate_tags
    ...
    for ticket in json_dict["tickets"]:
        ...
        write_ticket(csv_writer, ticket, candidate_tags)
