...
def ticket_to_csv(json_path, csv_path):
    ...
    for tagger in json_dict["taggers"]:
        ...
        if tagger["tagger_id"] != "0":
            human_taggers.append(tagger["tagger_id"])
    ...
    human_tagged_count = 0
    num_human_tagged = []
    human_nums = {}
    for i in range(1, len(human_taggers) + 1):
        human_nums[str(i)] = 0
    ...
    for ticket in json_dict["tickets"]:
        ...
        human_tagged = False
        human_count = 0
        for tagger in human_taggers:
            if tagger in ticket_taggers:
                human_count += 1
                human_tagged = True

        if human_tagged:
            human_tagged_count += 1
            num_human_tagged.append((ticket["ticket_id"], human_count))
    ...
    num_human_tagged.sort(key=lambda x: x[1])
    for i in num_human_tagged:
        human_nums[str(i[1])] += 1

    print("Human Tagged:", human_tagged_count)
    print(num_human_tagged)
    print(human_nums)
